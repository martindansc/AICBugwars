...
public abstract class MyUnit {
    ...
    MyUnit(UnitController unitController) {
        ...
        foodTracker = new FoodTracker(uc, 16, referenceLocation);
    ...
    /* This function returns 1 if the direction matches, 0.7 if it's a diagonal
     if it's going to the opposite direction then it returns -1 and -0.7 respectively
     else it's perpendicular, so 0 is returned
    */
    public double isDirectionTowardsLocation(Direction dir, Location loc) {
        Direction dirToLocation = uc.getLocation().directionTo(loc);
        if(dirToLocation.isEqual(dir)) {
            return 1;
        }
        if(dirToLocation.isEqual(dir.rotateLeft()) || dirToLocation.isEqual(dir.rotateRight())) {
            return 0.7;
        }


        Direction contraryDirection = dirToLocation.opposite();
        if(contraryDirection.isEqual(dir)) {
            return -1;
        }
        if(contraryDirection.isEqual(dir.rotateLeft()) || contraryDirection.isEqual(dir.rotateRight())) {
            return -0.7;
        }
